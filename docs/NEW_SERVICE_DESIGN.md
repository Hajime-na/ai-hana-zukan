...
| v1.3-rc3 | カフェ・店舗ポスター入口を「文字なしサンプルを選ぶ」「文字入れを依頼する」の2系統に分岐。①は既存ギャラリー（`#gallerySection`）をそのまま「文字なし」として流用し、購入・支払いまで即時セルフで完結させる |
| v1.3-rc4 | カフェ・店舗「文字入れオーダーメイド」の制作相談フォーム実装（用途・雰囲気ヒアリング → 掲載情報入力 → 制作相談を送信する）。想い出ポスターと同じ「制作相談」の扱いに統一 |
| v1.3-rc5 | `/admin` に制作相談一覧・詳細を追加（想い出ポスター／文字入れオーダーメイド共通）。ステータス（相談受付〜完了の10段階）の管理、写真・ヒアリング内容確認、サンプル送付の手動オペレーション導線を実装（サンプル生成・補正・文字入れ処理そのものは対象外） |
| v1.3-rc6 | 想い出ポスター・文字入れオーダーメイド双方の「サンプル確認 → 注文・支払い」導線を実装 |
| v1.3-rc7 | 管理画面（`/admin`）の独立ルート化。見積もり・支払い・入稿PDF・Prio発注メモを移設し、お客様向け画面から管理操作を完全撤去 |
| v1.3-rc8 | 旧UI要素（花図鑑・AI販促文・希望フォーム単独セクション等）の撤去、回帰確認、リリースノート整備 |
